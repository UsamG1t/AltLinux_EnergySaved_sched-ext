...
static int parse_task_token(const char *token, struct task_input *task)
{
	const char *p = token;
	char *endp;
	char short_name[SHEDULER_TASK_COMM_LEN];
	unsigned long id;
	...
	len = snprintf(short_name, sizeof(short_name), "task%u",
		       task->task_id);
	if (len < 0 || len >= SHEDULER_TASK_COMM_LEN)
		return -ENAMETOOLONG;
...
{
	const struct erf_task_plan *pa = a;
	const struct erf_task_plan *pb = b;
...
static int choose_task_frequency(struct erf_task_plan *plan, __u64 window_ns)
{
...
static int build_erf_schedule(const struct parsed_input *input,
			      struct erf_task_plan **plans_out,
			      struct erf_schedule_control *control)
{
	struct task_input *sorted_tasks;
	struct erf_task_plan *plans;
...
static void print_schedule_summary(const struct erf_task_plan *plans,
				   size_t nr_plans,
				   const struct erf_schedule_control *control)
{
	struct erf_task_plan *sorted;
	size_t i;
...
static int load_schedule_maps(struct scx_erf *skel,
			      const struct erf_schedule_control *control,
			      const struct erf_task_plan *plans,
			      size_t nr_plans)
{
	int task_plans_fd;
	int control_fd;
...
static void free_plans(struct erf_task_plan *plans)
{
	free(plans);
}
...
restart:
	skel = SCX_OPS_OPEN(erf_ops, scx_erf);
	SCX_OPS_LOAD(skel, erf_ops, scx_erf, uei);

	ret = load_schedule_maps(skel, &control, plans, input.nr_tasks);
	if (ret < 0)
		goto out;

	link = SCX_OPS_ATTACH(skel, erf_ops, scx_erf);
