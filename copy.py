...
def parse_freq_token(text: str) -> int:
    m = re.search(r"([0-9]+(?:\.[0-9]+)?)\s*(GHz|MHz|kHz)", text)
    if not m:
...
def start_fixed_cpuperf(fixed_cpuperf: Path, cpu: int, perf: int, cpuinfo_max_khz: int) -> subprocess.Popen:
...
def run_workload_and_sample(
    ...
    return {
        ...
        "scaling_khz": round(statistics.median(scaling_samples)) if scaling_samples else None,
...
def closest_step(freq_khz: int, available_steps: list[int]) -> tuple[int, int]:
    best = min(available_steps, key=lambda x: abs(x - freq_khz))
...
def main() -> int:
    ...
    ap.add_argument("--runtime-ms", type=int, default=400, help="workload duration per perf value")
    ap.add_argument("--sample-interval", type=float, default=0.03, help="cpufreq sample interval in seconds")
    ...
    ap.add_argument("--tolerance-khz", type=int, default=60_000, help="max deviation from available step")
    ...
    available = parse_cpupower_steps(args.cpu)

    print(f"CPU{args.cpu} available cpupower steps (kHz):")
    print(" ", available)
    print()
    ...
    with tempfile.TemporaryDirectory(prefix="perfscan-") as tmpdir:
        ...
        for perf in range(args.perf_min, args.perf_max + 1, args.perf_step):
            fixed_proc = None
            try:
                fixed_proc = start_fixed_cpuperf(fixed_cpuperf, args.cpu, perf, cpuinfo_max_khz)
    ...
    matched = [best_for_step[k] for k in sorted(best_for_step)]
    print()
