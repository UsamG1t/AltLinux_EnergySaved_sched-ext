...
def write_static_schedule_file(path: Path, deadline_ms: int, plans: Iterable[TaskPlan]) -> None:
    sorted_plans = sorted(plans, key=lambda plan: plan.task_id)

    with path.open("w", encoding="utf-8") as out:
        out.write(f"deadline_ms {deadline_ms}\n")
        out.write(f"nr_tasks {len(sorted_plans)}\n")
        out.write(
        ...
        for plan in sorted_plans:
            out.write(
                f"{plan.task_id} {plan.runtime_ms} {plan.ready_ms} {plan.cpu} "
                f"{plan.freq_step_idx} {plan.freq_khz} {plan.perf_target} "
                f"{plan.order} {plan.start_ns} {plan.duration_ns}\n"
            )
...
def launch_simulation(
    ...
    static_schedule_path = run_dir / "static_schedule.txt"
    ...
    scheduler_cmd = [
        ...
        str(static_schedule_path),
    ]
    print("Starting scheduler:", " ".join(scheduler_cmd))
    print(f"Static schedule written to {static_schedule_path}")
    ...
    release_plans = sorted(plans, key=lambda plan: (plan.ready_ms, plan.start_ns, plan.cpu, plan.order))
    ...
    try:
        for plan in release_plans:
            ...
            print(
                f"[+{plan.ready_ms:.3f} ms ready, +{plan.start_ns / 1_000_000:.3f} ms start] "
                f"launch {plan.name} cpu={plan.cpu} step={plan.freq_step_idx} freq={plan.freq_khz}"
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="ERF-based load simulator that pre-builds a static schedule for scx_scheduler"
    )
    parser.add_argument(
        "task_file",
        nargs="?",
    ...
    parser.add_argument(
        "--scheduler",
        type=Path,
        default=Path("./build/scx_scheduler/scx_scheduler"),
        help="Path to scx_scheduler binary",
    )
    parser.add_argument(
        "--origin",
        type=Path,
    ...
    parser.add_argument(
        "--run-dir",
        type=Path,
        default=Path("./build/scx_scheduler/run"),
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Sampling interval passed to scx_scheduler",
    )
    parser.add_argument(
        "--work-units-per-ms",
        type=int,
