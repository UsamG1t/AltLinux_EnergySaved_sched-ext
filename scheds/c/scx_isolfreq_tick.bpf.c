 ...
 * from. If scx_isolfreq_tick only supported global FIFO scheduling, then we could just
 * use SCX_DSQ_GLOBAL.
 */
#define SHARED_DSQ 0
#define ISOLATED_START 6
#define ISOLATED_END 9
#define PERF_STEP_NS 500000000ULL
...
/*
 * Update the shared sawtooth target from ops.tick() so a fully busy isolated
 * CPU keeps moving through the frequency range instead of waiting for
 * ops.dispatch(), which may not run again while the local DSQ stays occupied.
 */
static inline void update_isolated_perf(s32 cpu)
{
	...
	if (now - last >= PERF_STEP_NS) {
		...
		if (old == last) {
			...
			new_perf = target_perf;
		}
	}

	last_set = bpf_map_lookup_elem(&last_set_perf, &zero);
	if (last_set && *last_set != new_perf) {
		scx_bpf_cpuperf_set(cpu, new_perf);
		*last_set = new_perf;
	}
}

s32 BPF_STRUCT_OPS(isolfreq_tick_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
	bool is_idle = false;
	s32 cpu;
...
void BPF_STRUCT_OPS(isolfreq_tick_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if (!strncmp("var", name, 3)) {
		...
		u32 chosen = cpu_start + bpf_get_prandom_u32() % length;
...
void BPF_STRUCT_OPS(isolfreq_tick_dispatch, s32 cpu, struct task_struct *prev)
{
...
void BPF_STRUCT_OPS(isolfreq_tick_tick, struct task_struct *p)
{
	...
	update_isolated_perf(cpu);
}

void BPF_STRUCT_OPS(isolfreq_tick_running, struct task_struct *p)
{
	if (fifo_sched)
		return;
...
void BPF_STRUCT_OPS(isolfreq_tick_stopping, struct task_struct *p, bool runnable)
{
	if (fifo_sched)
		return;
...
void BPF_STRUCT_OPS(isolfreq_tick_enable, struct task_struct *p)
{
...
s32 BPF_STRUCT_OPS_SLEEPABLE(isolfreq_tick_init)
{
	...
	last_switch_time = bpf_ktime_get_ns();
    target_perf = 0;

	return scx_bpf_create_dsq(SHARED_DSQ, -1);
}

void BPF_STRUCT_OPS(isolfreq_tick_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}

SCX_OPS_DEFINE(isolfreq_tick_ops,
	       .select_cpu		= (void *)isolfreq_tick_select_cpu,
	       .enqueue			= (void *)isolfreq_tick_enqueue,
	       .dispatch		= (void *)isolfreq_tick_dispatch,
	       .tick			= (void *)isolfreq_tick_tick,
	       .running			= (void *)isolfreq_tick_running,
	       .stopping		= (void *)isolfreq_tick_stopping,
	       .update_idle		= (void *)isolfreq_tick_update_idle,
	       .enable			= (void *)isolfreq_tick_enable,
	       .init			= (void *)isolfreq_tick_init,
	       .exit			= (void *)isolfreq_tick_exit,
