...
const char help_fmt[] =
"A sheduler sched_ext scheduler.\n"
"\n"
...
static void sigint_handler(int sheduler)
{
	exit_req = 1;
}
...
{
	switch (event) {
	case SHEDULER_DEBUG_VAR_RUNNING:
		return "var";
	case SHEDULER_DEBUG_NONVAR_RUNNING_ZERO:
		return "run0";
	case SHEDULER_DEBUG_STOPPING_ZERO:
		return "stop0";
	case SHEDULER_DEBUG_IDLE_ZERO:
		return "idle0";
	default:
		return "-";
	}
...
static void format_task_comm(const char src[SHEDULER_COMM_LEN], char *dst, size_t size)
{
	size_t i;
	...
	for (i = 0; i + 1 < size && i < SHEDULER_COMM_LEN && src[i]; i++)
		dst[i] = src[i];
	dst[i] = '\0';
...
static int read_debug_states(struct scx_sheduler *skel,
			     struct sheduler_debug_cpu_state *states)
{
	...
	for (i = 0; i < NR_ISOLATED_CPUS; i++) {
		...
		states[i].last_var_comm[SHEDULER_COMM_LEN - 1] = '\0';
		states[i].last_actor_comm[SHEDULER_COMM_LEN - 1] = '\0';
	}
...
static void print_debug_sample(const struct sheduler_debug_cpu_state *states)
{
	int i;
...
static void write_csv_sample(FILE *csv, double elapsed_sec,
			     ...
			     const double *policy_mhz,
			     const struct sheduler_debug_cpu_state *debug_states)
{
	int i;
...
restart:
	...
	while (!exit_req && !UEI_EXITED(skel, uei)) {
		...
		struct sheduler_debug_cpu_state debug_states[NR_ISOLATED_CPUS] = {};
