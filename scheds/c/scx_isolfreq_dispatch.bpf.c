 ...
 * from. If scx_isolfreq_dispatch only supported global FIFO scheduling, then we could just
 * use SCX_DSQ_GLOBAL.
 */
#define SHARED_DSQ 0
...
s32 BPF_STRUCT_OPS(isolfreq_dispatch_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
	bool is_idle = false;
	s32 cpu;
...
void BPF_STRUCT_OPS(isolfreq_dispatch_enqueue, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(isolfreq_dispatch_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	if (cpu < ISOLATED_START || cpu > ISOLATED_END) {
        ...
        // Применяем новую частоту, если она изменилась
        u32 zero = 0;
        u32 *last_set = bpf_map_lookup_elem(&last_set_perf, &zero);
...
void BPF_STRUCT_OPS(isolfreq_dispatch_running, struct task_struct *p)
{
	if (fifo_sched)
		return;
...
void BPF_STRUCT_OPS(isolfreq_dispatch_stopping, struct task_struct *p, bool runnable)
{
	if (fifo_sched)
		return;
...
void BPF_STRUCT_OPS(isolfreq_dispatch_enable, struct task_struct *p)
{
	p->scx.dsq_vtime = vtime_now;
}

s32 BPF_STRUCT_OPS_SLEEPABLE(isolfreq_dispatch_init)
{
    u32 cpu;
    const struct cpumask* online = scx_bpf_get_online_cpumask();
...
void BPF_STRUCT_OPS(isolfreq_dispatch_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}

SCX_OPS_DEFINE(isolfreq_dispatch_ops,
	       .select_cpu		= (void *)isolfreq_dispatch_select_cpu,
	       .enqueue			= (void *)isolfreq_dispatch_enqueue,
	       .dispatch		= (void *)isolfreq_dispatch_dispatch,
	       .running			= (void *)isolfreq_dispatch_running,
	       .stopping		= (void *)isolfreq_dispatch_stopping,
	       .enable			= (void *)isolfreq_dispatch_enable,
	       .init			= (void *)isolfreq_dispatch_init,
	       .exit			= (void *)isolfreq_dispatch_exit,
	       .name			= "isolfreq_dispatch");
